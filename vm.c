...
int allocuvm(pde_t *pgdir, uint oldsz, uint newsz) {
  ...
  for(; a < newsz; a += PGSIZE){
    mem = kalloc();
    // Print the address of the bottom of the stack along with
    // the top of the user space to see if stack has grown
    // into the heap (top of user space, tracked by myproc()->sz
    if(mem == 0){
...
pde_t* copyuvm(pde_t *pgdir, uint sz) {
  ...
  // Copy over the stack that we created at the top of the user
  // space just below the KERNBASE
